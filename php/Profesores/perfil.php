...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark" >
    ...
    <br>
       ...
       $ruta= "../../img/";
            ...
            $listadodept = mysqli_query($connection, "SELECT * FROM tbl_professor where id_professor={$id};");            
        
            
            foreach ($listadodept as $prof) {
                $sqllook = mysqli_query($connection, "SELECT nom_dept FROM tbl_dept where id_dept = {$prof['dept']}");
                $sqlnom = mysqli_fetch_array($sqllook);
                echo '<tr class="Fondo_Perfil">';  
                    echo '<div class="campos1">';
                        echo '<div class="parte1">';
                            echo "<td ><img class='FotoPerfil1' src=$ruta"."{$prof['foto_prof']}></td>";  
                            
                        echo '</div>';
                        echo '<div class="parte2">';                       
                            echo "<td><p class='datos3'>Nombre</p></td>";                  
                            echo "<td><p class='datos2'>{$prof['nom_prof']}</p></td>";                  
                            echo "<td><p class='datos3'>1r Apellido</p></td>";                
                            echo "<td><p class='datos2'>{$prof['cognom1_prof']}</p></td>";                  
                            echo "<td><p class='datos3'>2n Apellido</p></td>";
                            echo "<td><p class='datos2'>{$prof['cognom2_prof']}</p></td>"; 
                        echo '</div>';  
                        
                        echo '<div class="parte3">';                                           
                            echo "<td><p class='datos3'>Telefono</p></td>";                    
                            echo "<td><p class='datos2'>{$prof['telf']}</p></td>";                
                            echo "<td><p class='datos3'>Email</p></td>";                
                            echo "<td><p class='datos2'>{$prof['email_prof']}</p></td>";                
                            echo "<td><p class='datos3'>Clase</p></td>";                 
                            echo "<td><p class='datos2'>{$sqlnom['nom_dept']}</p></td>";   
                        echo '</div>'; 
                    echo '</div>'; 
